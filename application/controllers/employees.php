...
class Employees extends CI_Controller {
    ...
    function login() {
        if ($this->session->userdata('employeeId'))
            redirect('employees/batchpayment');
    ...
    public function batchpayment() {
        ...
        if ($user_detail[0]['address_update_date'] == "" or $to >= strtotime($user_detail[0]['address_update_date'])) {
            redirect('/employees/changeAddress');
        }
        $config['base_url'] = base_url() . "employees/batchpayment";
        $config['total_rows'] = $this->payment_model->countPaymentsEmployee();
        $config['per_page'] = $this->limit;
        $config['uri_segment'] = 2;
        $payment = $this->payment_model->paymentListEmployee($config['per_page'], $this->uri->segment(3));
    ...
    public function updateNewAddress() {
        ...
        echo json_encode(array('redirect' => HTTP_PATH . 'employees/myProfile'));
    }
    ...
    public function updateAddress() {
        ...
        if ($address) {
            $this->payment_model->updateAddress(array('rental_type' => 'company', 'non_comp_address' => "", 'house_id' => $address, 'address_update_date' => date('Y-m-d')));
            $this->session->set_userdata('smessage', "Company address successfully updated");
            redirect('employees/myProfile');
        } else {
            $this->payment_model->updateAddress(array('rental_type' => 'non-company', 'non_comp_address' => $searched_address, 'house_id' => 0, 'address_update_date' => date('Y-m-d')));
            $this->session->set_userdata('smessage', "Non company address successfully updated");
            redirect('employees/myProfile');
        }
    }
