...
if (!empty($users)) {
    ...
    foreach ($users as $row) {
        ...
        <?php
        if ($row->probation == 'yes') {
            echo " probation-heightlight";
        }
        ?>">
            <div class="phone_no phone_no26">
            ...
            <div class="action action21">
                ...
                if ($this->uri->segment(2) <> 'employeeInWorksite' and $this->uri->segment(2) <> 'employeeInHouse' and $user_detail['role'] <> 4) {
                    ...
                    ?> 
                    <?php
                    echo anchor('/users/deleteUser/' . $row->id, "<img src=" . HTTP_PATH . "img/icon2.png" . " width='16' height='16' border='0'>", array('title' => 'Delete', 'class' => "delete"));
                }
                ?>
            </div>
        </div>	
