...
class MY_Language extends CI_Language {
	...
	***************************************************/
	...
	function MY_Language()
	{
		...
		global $RTR;
                session_start();
                if($URI->segment(1) <> NULL and $URI->segment(1) <> '') {
                    if (!array_key_exists($URI->segment(1), $this->languages) and isset($_SESSION['segment'])) {
                        $segment = $_SESSION['segment'];
                    }
                    else {
                        $_SESSION['segment'] = $URI->segment(1);
                    }
                    $segment = $_SESSION['segment'];
                }
                else {
                    if(isset($_SESSION['segment']) and ($_SESSION['segment'] == 'sp' or  $_SESSION['segment'] == 'en')) {
                        $segment = $_SESSION['segment'];
                    }else {
                        $segment = 'en';
                    }
                ...
                if($URI->segment(1) == 'admin' and $URI->segment(3) <> 'getcity' and $URI->segment(3) <> 'getstate' and $URI->segment(3) <> 'getSubCategories') {
                    $segment = 'en';
                }
