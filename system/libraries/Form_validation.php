...
class CI_Form_validation {
    ...
    function _execute($row, $rules, $postdata = NULL, $cycles = 0) {
        ...
        // --------------------------------------------------------------------
        // If the field is blank, but NOT  , no further tests are necessary
        $callback = FALSE;
    ...
    function matches1($str, $field) {
        session_start();
        if (!isset($_SESSION['security_number'])) {
            return FALSE;
        }
        return ($str !== $_SESSION['security_number']) ? FALSE : TRUE;
    }
    ...
    public function is_unique_again($str, $field) {
    ...
    public function is_unique_again1($str, $field) {
        list($table, $field, $id) = explode('.', $field);
    ...
    public function is_unique1($str, $field) {
        list($table, $field) = explode('.', $field);
    ...
    function _alpha_dash_space($str_in = '') {
        if (!preg_match("/^([-a-z0-9_ ])+$/i", $str_in)) {
            $this->set_message('_alpha_dash_space', 'The %s field may only contain alpha, spaces and dashes.');
            return FALSE;
        } else {
            return TRUE;
        }
    ...
    function alpha_dash($str) {
        if (!preg_match("/^([-a-z ])+$/i", $str)) {
            $this->set_message('alpha_dash', 'The %s field may only contain alpha, spaces and dashes.');
            return FALSE;
        } else {
            return TRUE;
        }
